...
\author{Zhibo Wang and Kaigui Wu\textsuperscript{\Letter} \\ School of Computer Science, Chongqing University, Chongqing, 400044, China\\zhibowang@stu.cqu.edu.cn, kaiguiwu@cqu.edu.cn}
...
\begin{figure*}
\includegraphics[width=\linewidth]{Figures/MSL-UNet.pdf}
\caption{
	...
	(d) Multi-scale Strip Large-kernel convolutional block(MSLCB), progressing patches in all dimensions to further enhance features, is the main component of our decoder.
...
As the front branch loses local features through global modeling, we preserve local features in the other branch, where patches will only undergo a linear layer $LL(.)$ and an activation function $R(.)$.
...
\textbf{Channel Attention Block (CA)}\cite{woo2018cbam}: As depicted in Figure \textcolor{red}{1} (f), the input patches $x$ first enter the channel attention (CA) module and are then split into two branches, where $x$ undergo adaptive max pooling\cite{he2014spatial} ($AMP(\cdot)$) and adaptive average pooling ($AAP(\cdot)$)\cite{he2014spatial} respectively, to extract the most significant feature per channel from the entire feature map. For each pooled feature map, we apply two point-wise convolutions\cite{lin2013network}: $PWC_{C/r}(\cdot)$ (with $r=16$) to reduce the number of channels and $PWC_C(\cdot)$ to recover it, with an activation function $R(\cdot)$ in between. The two pooled feature maps are then combined via bit-wise addition before passing through an activation function $\sigma(\cdot)$. Finally, we perform a bit-wise multiplication (Hadamard product) between the combined feature map and the initial input $x$.
...
\textbf{Multi-scale Attention Block (MA)}: As depicted in Figure \textcolor{red}{1} (h), the input patches $x$ first undergo channel expansion via a point-wise convolution $PWC_{C\times 2}(\cdot)$ (with an expansion factor of 2), followed by batch normalization $BN(\cdot)$ and an activation function $R6(\cdot)$ (ReLU6\cite{howard2017mobilenets}). Then, the patches are separately passed through three depthwise separable convolution layers ($DWC(\cdot)$) with different kernel sizes to capture multi-scale and multi-resolution contexts (the number and size of the kernels are configurable). After the features from different channels are summed, we apply a channel shuffle layer\cite{zhang2018shufflenet} $CSF(\cdot)$ to enhance cross-channel relationships. The shuffled feature maps are then passed through another point-wise convolution $PWC_C(\cdot)$ to recover the original number of channels, followed by batch normalization $BN(\cdot)$. Finally, the normalized feature maps are added bit-wise to the initial input $x$.
...
Strip Convolution Block (SC)\cite{hou2020strip,liu2023slak,chu2021active} is integrated into our MSL-UNet to address the limitations of traditional square-kernel convolutions in capturing elongated and curvilinear anatomical structures.

SC is designed as shown in Figure \textcolor{red}{1} (e). The input $x$ first passes through a layer normalization, a fully connected layer, and a GELU activation function\cite{hendrycks2016gelu}. The normalized patches $x'$ then enter the Strip Module, which is the core part of SC depicted in Figure \textcolor{red}{1} (j).
...
We compare our MSL-UNet with many state-of-the-art models on Synapse and ACDC datasets. We choose models with different type of architecture. Pure CNN models like UNet\cite{ronneberger2015unet}. Pure Transformer models like Swin-UNet\cite{cao2021swinunet,cao2023swin}. Pure Mamba models like VM-UNet\cite{ruan2024vmunet}. Hybrid architecture models like TransUNet\cite{chen2021transunet} that uses CNN and Transformer. And other SOTA models such as TransCASCADE\cite{rahman2023transcascade} and so on. The later result details indicate our MSL-UNet achieves best among the SOTAs.



\subsubsection{Results on Synapse Multi-Organ Segmentation}

Results of experiments on Synapse are presented in Table \textcolor{red}{1}. Among SOTAs our MSL-UNet achieves the best performance, boasting a mean DICE score of 83.56 and a mean HD95 of 16.18, surpassing TransCASCADE\cite{rahman2023transcascade} and VM-UNet\cite{ruan2024vmunet}. We believe two facts help our model, one is Mamba encoder costs less resources but predicts more precisely, the other one is our MSL-UNet employs large-kernel and strip convolution that successfully enhance capacity of feature extraction. Figure \textcolor{red}{2} shows visual results of some SOTA models on Synapse segmentation. Evidently, several SOTA models exhibit misclassifications. TransUNet\cite{chen2021transunet}, Swin-UNet\cite{cao2021swinunet}, TransDeepLab\cite{azad2022transdeeplab} and VM-UNet\cite{ruan2024vmunet} miss left kidney. TransUNet\cite{chen2021transunet} and Swin-UNet\cite{cao2021swinunet} even lost very big part of pancreas. A similar limitation is observed in TransDeepLab\cite{azad2022transdeeplab} but not serious like the former two. Our MSL-UNet performs best, especially in segmentation on tiny and slender organs such as both right and left kidney.
