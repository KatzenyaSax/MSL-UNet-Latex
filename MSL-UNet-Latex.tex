...
For the encoder, we employ eight VSS Blocks in four stages where one stage pocesses two VSS Blocks, with a patch merging operation at the end of the first three ges to reduce the height and width of the input features while increasing the number of channels. The channel counts for each stage are [C, 2C, 4C, 8C].

Similarly, for the decoder we employ four MSLCBs to refine pyramid features extracted from the encoder via skip connections(i.e., x1, x2, x3, x4 in Figure1). Also, a patch expanding operation is used at the end of the last three MSLCBs. The channel counts for each stage are [8C, 4C, 2C, C].

We employ three SLGAGs for the skip connections. Features that are upsampled in patch expanding layer are added to the output from SLGAGs. 

Finally, the SH layer produces the final segmentation output.

\subsection{VSS Block}

As depicted in Figure \textcolor{red}{1} (b), the VSS Block, derived from VMamba\cite{VMamba}, is the core module of our encoder. Firstly, the input patches $x$ are layer normalized, then split into two branches, where one branch performs global modeling but loses local features, and the other branch preserves local features.
...
\bibitem{chen2021transunet}Chen, J., Lu, Y., Yu, Q., Luo, X., Adeli, E., Wang, Y., ... \& Zhou, Y. (2021). TransUNet: Transformers make strong encoders for medical image segmentation. arXiv preprint arXiv:2102.04306. 

\bibitem{dosovitskiy2020vit} Dosovitskiy, A., Beyer, L., Kolesnikov, A., Weissenborn, D., Zhai, X., Unterthiner, T., ... \& Houlsby, N. (2020). An image is worth 16x16 words: Transformers for image recognition at scale. arXiv preprint arXiv:2010.11929 (Published at ICLR 2021).
