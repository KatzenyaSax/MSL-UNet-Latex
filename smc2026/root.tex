...
\begin{enumerate}
\item We introduce SLMambaMedseg that synergistically combines the global modeling strength of a Vision Mamba encoder with the localized, detail-oriented capacity of a heavily augmented convolutional decoder. 
...
\item Extensive experiments on Synapse and ACDC demonstrate that our SLMambaMedseg achieves state-of-the-art performance with markedly improved efficiency, offering a compelling solution for practical medical image analysis.
\end{enumerate}
...
SLMambaMedseg have compared with many state-of-the-art models with different type of architecture on Synapse and ACDC datasets. Pure CNN models like UNet\cite{ronneberger2015unet}. Pure Transformer models like Swin-UNet\cite{cao2021swinunet,cao2023swin}. Pure Mamba models like VM-UNet\cite{ruan2024vmunet}. Hybrid architecture models like TransUNet\cite{chen2021transunet} that uses CNN and Transformer. And other SOTA models such as TransCASCADE\cite{rahman2023transcascade} and so on. The later result details indicate our SLMambaMedseg achieves best among the SOTAs.
