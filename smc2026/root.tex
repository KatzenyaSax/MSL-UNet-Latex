...
\title{\LARGE \bf
%SLMambaMedseg: Synergizing Vision Mamba and Multi-scale Strip Large-kernel Convolutions for Medical Image Segmentation%*
EMSLD : Efficient Multiple Strip Large-Kernel Decocder for Medical Segmentation 

}


\author{Zhibo Wang$^{1}$ and Kaigui Wu$^{2}$\textsuperscript{\Letter}% <-this % stops a space
%\thanks{*This work was not supported by any organization}% <-this % stops a space
\thanks{$^{1}$Zhibo Wang is with School of Computer Science, Chongqing University, Chongqing, 400044, China
        {\tt\small zhibowang@stu.cqu.edu.cn}}%
\thanks{$^{2}$Kaigui Wu is School of Computer Science, Chongqing University, Chongqing, 400044, China
        {\tt\small kaiguiwu@cqu.edu.cn}}%
}
...
Automated segmentation of anatomical structures in medical images is foundational for computer-aided diagnosis and treatment planning\cite{ronneberger2015unet,milletari2016vnet,isensee2021nnu}. While Convolutional Neural Networks (CNNs) long dominated this field with U-Net, their inherent limitation in modeling long-range dependencies has motivated the shift to self-attention mechanisms\cite{chen2021transunet,cao2021swinunet}. However, the quadratic complexity of Transformers imposes significant computational burden. Vision Mamba\cite{gu2023mamba,liu2024vmamba,zhu2024vim}, built on selective state space models (SSMs), has recently emerged as a compelling alternative, achieving global context modeling with linear complexity.

Vision Mamba has been integrated as an encoder in segmentation frameworks\cite{ruan2024vmunet,ma2024umamba}. Yet a critical gap persists: Mamba's strength lies in modeling rather than generation, so existing models predominantly pair it with separate decoders\cite{Smamba,MaIL}, leaving the full potential of a unified Mamba-centric architecture untapped.

To bridge this gap, we propose SLMambaMedseg, a hybrid architecture integrating a Vision Mamba encoder with a comprehensively enhanced convolutional decoder. The encoder employs hierarchical VSS blocks for efficient global representation. The decoder introduces a Multi-scale Strip Large-kernel Convolutional Block (SLCB) and a Strip Large-Kernel Grouped Attention Gate (SLGAG). Inspired by large-kernel CNNs\cite{liu2022convnext} and strip convolutions\cite{hou2020strippooling}, SLCB uses parallel strip and large-kernel depthwise separable convolutions to capture anisotropic features and broad context simultaneously. SLGAG refines skip-connection fusion, preserving spatial details during upsampling.

The principal contributions are:
\begin{enumerate}
\item We introduce SLMambaMedseg that synergistically combines a Vision Mamba encoder with a heavily augmented convolutional decoder.
\item We design SLCB and SLGAG, which systematically incorporate strip and large-kernel convolutions to address the geometric characteristics of medical organs.
\item Extensive experiments on Synapse and ACDC demonstrate state-of-the-art performance with markedly improved efficiency.
\end{enumerate}
...
Medical image segmentation has been driven by the trade-off between local feature extraction and global context modeling. U-Net\cite{ronneberger2015unet}, built on CNNs, established the de facto standard, but the intrinsic locality of convolution restricts long-range dependency modeling\cite{luo2016erf,wang2018nonlocal}, limiting performance on structures with large scale variations\cite{chen2021transunet}.

Vision Transformers (ViTs)\cite{dosovitskiy2020vit} addressed this by adopting self-attention for global context. TransUNet\cite{chen2021transunet}, Swin-UNet\cite{cao2021swinunet}, and SegFormer\cite{xie2021segformer} have demonstrated strong results in medical segmentation using Transformer-based designs.

However, the quadratic complexity of self-attention imposes heavy computational burden\cite{vaswani2017attention}, spurring interest in hybrid architectures that balance efficiency and performance\cite{xie2021cotr}. TransUNet\cite{chen2021transunet} exemplifies this paradigm by combining a CNN encoder with Transformer bottleneck. Our SLMambaMedseg follows this hybrid philosophy, further replacing the Transformer with the more efficient Vision Mamba.

\subsection{State Space Models}

State Space Models (SSMs) capture long-range dependencies with linear complexity, contrasting sharply with the quadratic scaling of self-attention\cite{gu2023mamba,vaswani2017attention}. The Structured State Space Sequence model (S4)\cite{gu2021efficiently} integrated the HiPPO framework\cite{gu2020hippo} for long-range memory, enabling efficient parallel training. However, S4's time-invariant parameters remain independent of input data\cite{fu2022hungry}, limiting context-dependent reasoning critical for dense visual tasks.

Mamba\cite{gu2023mamba} overcame this by introducing input-dependent transition matrices, enabling selective information propagation. This allows Mamba-based architectures to match Transformer-level representation while retaining linear complexity, making them a compelling alternative for medical vision tasks\cite{zhu2024vim,liu2024vmamba,ruan2024vmunet}.
...
This section will first introduce the overall architecture of SLMambaMedseg, followed by a detailed description of each individual module used within it, including various technical principles, implementation details, and parameter settings.

\subsection{SLMambaMedseg}

As depicted in Figure \textcolor{red}{\ref{figure1}} (a), the overall architecture of SLMambaMedseg is presented, which is based on U-Shape-Network\cite{ronneberger2015unet}. Specifically, SLMambaMedseg comprises a Patch Embedding layer as initial processing, four-stage VSS Blocks\cite{liu2024vmamba,gu2023mamba} as the encoder, four-stage SLCBs as the decoder, a SH layer as final projection and SLGAGs as skip connections.
...
\textbf{Channel Attention Block (CA)}\cite{woo2018cbam}: As depicted in Figure \textcolor{red}{\ref{figure1}} (f), the input patches $x$ first enter the channel attention (CA) module and are then split into two branches, where $x$ undergo adaptive max pooling ($AMP(\cdot)$) and adaptive average pooling ($AAP(\cdot)$) respectively, to extract the most significant feature per channel from the entire feature map. For each pooled feature map, two point-wise convolution blocks are applied: $PWC_{C/r}(\cdot)$ (with $r=16$) to reduce the number of channels and $PWC_C(\cdot)$ to recover it, with an activation function $R(\cdot)$ in between. The two pooled feature maps are then combined via bit-wise addition before passing through an activation function $\sigma(\cdot)$. Finally, a bit-wise multiplication is applied between the combined feature map and the initial input $x$ to get the final output.
...
\textbf{Multi-scale Attention Block (MA)}: As depicted in Figure \textcolor{red}{\ref{figure1}} (h), the input patches $x$ first undergo channel expansion via a point-wise convolution $PWC_{C\times 2}(\cdot)$ (with an expansion factor of 2), followed by batch normalization $BN(\cdot)$ and an activation function $R6(\cdot)$ (ReLU6). Then, the patches are separately passed through three depthwise separable convolution layers ($DWC(\cdot)$) with different kernel sizes to capture multi-scale and multi-resolution contexts (the number and size of the kernels are configurable). After the features from different channels are summed, a channel shuffle layer $CSF(\cdot)$ will be applied to enhance cross-channel relationships. The shuffled feature maps are then passed through another point-wise convolution $PWC_C(\cdot)$ to recover the original number of channels, followed by batch normalization $BN(\cdot)$. Finally, the normalized feature maps are added bit-wise to the initial input $x$.

The $MA(.)$ can be formulated as in Equation \textcolor{red}{5}:
\begin{scriptsize}   
\begin{align}\nonumber
  MA(x)=x+BN(PWC_{C}(CSF(\sum_{}^{}DWC_{k_i}\\ 
  (R6(BN(PWC_{C\times2}(x)))))))
\end{align}
\end{scriptsize}
...
Strip Convolution Block (SC)\cite{hou2020strip,liu2023slak,chu2021active} is integrated into our SLMambaMedseg to address the limitations of traditional square-kernel convolutions in capturing elongated and curvilinear anatomical structures.

SC is designed as shown in Figure \textcolor{red}{\ref{figure1}} (e). The input $x$ first passes through a layer normalization, a fully connected layer, and a GELU activation function. The normalized patches $x'$ then enter the Strip Module, which is the core part of SC depicted in Figure \textcolor{red}{\ref{figure1}} (j).
...
SLMambaMedseg have compared with many state-of-the-art models with different type of architecture on Synapse and ACDC datasets. Pure CNN models like UNet\cite{ronneberger2015unet}. Pure Transformer models like Swin-UNet\cite{cao2021swinunet,cao2021swinunet}. Pure Mamba models like VM-UNet\cite{ruan2024vmunet}. Hybrid architecture models like TransUNet\cite{chen2021transunet} that uses CNN and Transformer. And other SOTA models such as TransCASCADE\cite{rahman2023transcascade} and so on. The later result details indicate our SLMambaMedseg achieves best among the SOTAs.



\subsubsection{Results on Synapse Multi-Organ Segmentation}

Results of experiments on Synapse are presented in Table \textcolor{red}{\ref{table1}}. Among SOTAs our SLMambaMedseg achieves the best performance, boasting a mean DICE score of 83.56 and a mean HD95 of 16.18, surpassing TransCASCADE\cite{rahman2023transcascade} and VM-UNet\cite{ruan2024vmunet}. There are two facts help our model, one is Mamba encoder costs less resources but predicts more precisely, the other one is our SLMambaMedseg employs large-kernel and strip convolution that successfully enhance capacity of feature extraction. Figure \textcolor{red}{\ref{figure2}} shows visual results of some SOTA models on Synapse segmentation. Evidently, several SOTA models exhibit misclassifications. TransUNet\cite{chen2021transunet}, Swin-UNet\cite{cao2021swinunet}, TransDeepLab\cite{azad2022transdeeplab} and VM-UNet\cite{ruan2024vmunet} miss left kidney. TransUNet\cite{chen2021transunet} and Swin-UNet\cite{cao2021swinunet} even lost very big part of pancreas. A similar limitation is observed in TransDeepLab\cite{azad2022transdeeplab} but not serious like the former two. Our SLMambaMedseg performs best, especially in segmentation on tiny and slender organs such as both right and left kidney.
...
\hline
SLMambaMedseg(ours) & \textbf{92.81} & \textbf{91.52} & \textbf{90.58} &\textbf{96.53} \\\hline
\end{tabular}
...
SLMambaMedseg & \textbf{83.36} & \textbf{16.18} & \textbf{92.81} \\\hline
\end{tabular}
...
To understand the effect of different encoders, a set of experiments are conducted on the Synapse and ACDC datasets. Two extra version of SLMambaMedseg, SLCNNMedseg and SLTransMedseg by replacing the encoder using CNN(as UNet\cite{ronneberger2015unet}) and Transformer(as SwinUNet\cite{cao2021swinunet}), have been built up tp be compared. Table \textcolor{red}{\ref{table3}} shows the experimental results. Our SLMambaMedseg with vision mamba encoder have achieved the best scores on both the Synapse and ACDC datasets, surpassing SLCNNMedseg and SLTransMedseg. Such results are completely in line with our expectations. The capability of CNNs in processing image features can no longer meet the surging demands, and their performance falls short when compared to Transformers and Mamba. However, Transformers also have limitations, as an excessive amount of data not only leads to a sharp increase in parameter counts but may also negatively impact segmentation results. In contrast, the Mamba encoder's ability to compress data in long-sequence\cite{yu2024mambaout} tasks is likely the key factor that enables it to outperform Transformers.
...
SLMambaMedseg & \textbf{83.36} & \textbf{16.18} &\textbf{88.57}  & \textbf{92.81} \\\hline
\end{tabular}
...
To observe the actual effectiveness of our SLGAG, one specifical experiment has been designed by removing the skip connections composed of SLGAG from SLMambaMedseg and use unprocessed skip connections instead, comparing this variant with our existing SLMambaMedseg on the Synapse and ACDC datasets. The experimental results are shown in Table \textcolor{red}{\ref{table4}}. The SLMambaMedseg with SLGAG achieves higher scores than the model without SLGAG, particularly demonstrating a significant lead in segmentation tasks involving small and elongated structures, such as the left kidney on the Synapse dataset. This indicates that the strip convolutions in our SLGAG can effectively capture relatively small and elongated features.
